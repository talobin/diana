...
public class ExternalNote {
    ...
    public ExternalNote(String translatedNote, int octave, double frequency, double actualFrequency, double offset, TunerOptions tunerOptions) {
    ...
    public static ExternalNote parse(String s, TunerOptions tunerOptions) {
        ...
        return new ExternalNote(translatedNote, octave, frequency, frequency, 0, tunerOptions);
    }

    public static ExternalNote parse(double frequency, TunerOptions tunerOptions) {
        double aboveA =
                Math.log(
        ...
        return new ExternalNote(note, octave, closest, frequency, offset, tunerOptions);
    }

    public double offsetFrom(ExternalNote n) {
        double nAboveA =
                Math.log(
    ...
    public static String getNotes(String s, TunerOptions tunerOptions) {
        ...
        for(int i = 0; i < notes.length; i++) {
            notesString.append(ExternalNote.parse(notes[i], tunerOptions).getTranslatedNote());
            notesString.append(ExternalNote.parse(notes[i], tunerOptions).getOctave());
            notesString.append(" ");
        }
